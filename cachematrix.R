## This script provides functions to construct and access the special square
## matrix object able to cache it's inverse to avoid potentially 
## time-consuming inverse re-calculations 

# obj <- makeCacheMatrix(x = matrix())  constructs object from 
# square matrix x that can hold x's inverse
...
cacheSolve <- function(x, ...) {
    ...
    inv <- x$getinverse()
    # if it exsists/NOT NULL, returning it, avoiding re-calculatiogn
    if(!is.null(inv)) {
